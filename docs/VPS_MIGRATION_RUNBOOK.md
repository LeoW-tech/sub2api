...
不要在旧实例仍提供生产服务时长期启动 VPS 上的 Sub2API。应用启动后会自动运行令牌刷新、邮件队列、定时测试和监控等后台任务；两个独立数据库上的实例同时运行可能争用同一批上游账号状态。
...
需要临时验证应用时，只在明确接受后台任务短暂启动的情况下执行：
...
5. 执行 `sudo systemctl enable --now sub2api-stable.service` 启动 VPS 的 Sub2API，并验证健康、管理登录、固定令牌、代表性上游请求和重启恢复。
