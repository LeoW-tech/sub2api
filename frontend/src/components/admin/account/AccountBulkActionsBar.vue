<template>
  ...
  >
    <div class="flex flex-wrap items-center gap-2">
      ...
      <template v-if="!allResultsSelected && (totalResults ?? 0) > selectedIds.length">
        ...
        >
          {{
            selectingAll
              ...
              : t('admin.accounts.bulkActions.selectAllResults', { count: totalResults ?? 0 })
          }}
        </button>
      </template>
      <template v-if="selectedIds.length > 0">
    ...
    <div class="flex gap-2">
      <template v-if="selectedIds.length > 0">
        ...
        >
          {{
            testingActivate
              ? t('admin.accounts.bulkActions.testActivating')
