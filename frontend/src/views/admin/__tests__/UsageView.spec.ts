...
const { list, exportList, getStats, getSnapshotV2, getById, getModelStats, listErrorLogs, routeQuery, aoaToSheet, sheetAddAoa, saveAs, xlsxWrite, buildUsageWorkbookBlob } = vi.hoisted(() => {
  ...
  return {
    ...
    buildUsageWorkbookBlob: vi.fn(async () => new Blob(['xlsx'])),
  }
})
...
vi.mock('@/utils/usageExportWorkbook', () => ({ buildUsageWorkbookBlob }))
...
describe('admin UsageView model audit export', () => {
	beforeEach(() => {
		...
		saveAs.mockClear()
		xlsxWrite.mockClear()
		buildUsageWorkbookBlob.mockClear()
	})
	...
	it('exports requested, sent, response, and mismatch as separate admin columns', async () => {
		...
		const [headers, rows] = buildUsageWorkbookBlob.mock.calls[0]
