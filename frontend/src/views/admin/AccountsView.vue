...
function getOpenAICompactState(row: any): 'active' | 'blocked' | 'auto' | null {
  ...
  const extra = row.extra as Record<string, unknown> | undefined
  const mode = normalizeOpenAICompactMode(extra?.openai_compact_mode)
...
function getOpenAICompactLabel(row: any): string | null {
  return getOpenAICompactMeta(row)?.label || null
}

function getOpenAICompactClass(row: any): string {
  return getOpenAICompactMeta(row)?.className || ''
}
...
const allColumns = computed(() => {
  ...
  c.push(
    ...
    {
      key: "network_status",
      label: t("admin.accounts.columns.networkStatus"),
      sortable: false,
    },
...
const handleBulkTestActivate = async () => {
  ...
  if (
    !confirm(
      t("admin.accounts.bulkActions.testActivateConfirm", {
...
const handleBulkToggleSchedulable = async (schedulable: boolean) => {
  ...
  try {
    ...
    const {
      ...
      hasIds,
      hasCounts,
    } = normalizeBulkUpdateResult(result, accountIds);
    if (!hasIds && !hasCounts) {
...
const accountMatchesCurrentFilters = (account: Account) => {
  ...
  if (filters.ip) {
    if (account.proxy?.exit_ip !== filters.ip) return false;
  }
...
onMounted(async () => {
  ...
  const [proxiesResult, groupsResult, ipOptionsResult] = await Promise.allSettled([
    ...
    typeof adminAPI.proxies.getIPOptions === "function"
