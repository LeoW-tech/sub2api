...
describe('help page locales', () => {
  it('defines navigation and help page copy in Chinese and English', () => {
    ...
    expect(zh.help.steps.install.title).toBe('下载安装 Codex')
    ...
    expect(zh.help.steps.learn.title).toBe('学习使用')
    expect(zh.help.links.subscriptions).toBe('查看我的订阅')
    expect(zh.help.screenshots.key.name).toBe('Codex 密钥')

    expect(en.help.steps.install.title).toBe('Download and install Codex')
    ...
    expect(en.help.steps.learn.title).toBe('Learn to use Codex')
