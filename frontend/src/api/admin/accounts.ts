...
export async function list(
  ...
  const { data } = await apiClient.get<PaginatedResponse<AccountListItem>>(
    "/admin/accounts",
    {
      params: {
        page,
...
export async function getUpstreamBillingRatesWithEtag(
  ...
  const response = await apiClient.get<UpstreamBillingRatesResponse>(
...
export async function refreshCredentials(id: number): Promise<RefreshCredentialsResult> {
  const { data } = await apiClient.post<Account | RefreshCredentialsResult>(
...
export async function exportData(options?: {
  ...
  if (options?.ids && options.ids.length > 0) {
    ...
    if (network_status) params.network_status = network_status;
    if (ip) params.ip = ip;
...
export async function setOllamaCloudUsageAutoRefresh(id: number, enabled: boolean): Promise<OllamaCloudUsageState> {
	const { data } = await apiClient.put<OllamaCloudUsageState>(`/admin/accounts/${id}/ollama-cloud-usage/auto-refresh`, { enabled });
	return data;
}
