...
func (h *AccountHandler) listAccountSchedulerScoreFilterPool(
	...
	accounts, err := h.adminService.ListAccountsForSchedulerScoreFilter(ctx, service.PlatformOpenAI, accountType, status, search, groupID, privacyMode, networkStatus, exitIP, rtStatus, capacityStatus)
	if err != nil {
...
func (h *AccountHandler) List(c *gin.Context) {
	...
	if lite {
		...
		etag := buildAccountsListETag(compact, total, page, pageSize, platform, accountType, status, search, groupID, privacyMode, networkStatus, exitIP, rtStatus, capacityStatus, true)
		if etag != "" {
			c.Header("ETag", etag)
	...
	etag := buildAccountsListETag(result, total, page, pageSize, platform, accountType, status, search, groupID, privacyMode, networkStatus, exitIP, rtStatus, capacityStatus, false)
	if etag != "" {
		c.Header("ETag", etag)
		c.Header("Vary", "If-None-Match")
		if ifNoneMatchMatched(c.GetHeader("If-None-Match"), etag) {
...
func (h *AccountHandler) TrueRefresh(c *gin.Context) {
	...
	// #nosec G704 -- baseURL is an operator-controlled internal true-refresh endpoint.
	...
	// #nosec G704 -- request target is the validated operator-controlled true-refresh endpoint above.
	...
	defer func() {
		if closeErr := httpResp.Body.Close(); closeErr != nil {
			log.Printf("close true-refresh response body failed: %v", closeErr)
		}
	}()
