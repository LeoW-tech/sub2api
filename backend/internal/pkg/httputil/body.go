...
// readRequestBodyChunks bounds each allocation while allowing the buffer to
// grow with the bytes that actually arrive, avoiding eager allocation from an
...
func readRequestBodyChunks(reader io.Reader, initialCapacity int, contentLength int64) ([]byte, error) {
	...
	for {
		...
		if err != nil && err != io.EOF {
			return nil, &BodyReadError{Err: err, BytesRead: int64(total + n)}
		}
		if n > 0 {
