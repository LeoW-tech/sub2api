...
type Config struct {
	...
	ProxyNetworkMonitor     ProxyNetworkMonitorConfig     `mapstructure:"proxy_network_monitor"`
...
// ProxyNetworkMonitorConfig 代理网络检测后台任务配置
type ProxyNetworkMonitorConfig struct {
	// 是否启用自动网络检测任务
	Enabled bool `mapstructure:"enabled"`
}
