...
type AdminService interface {
	...
	ListAccountsForSchedulerScoreFilter(ctx context.Context, platform, accountType, status, search string, groupID int64, privacyMode, networkStatus, exitIP, rtStatus, capacityStatus string) ([]Account, error)
	...
	GetProxyIPOptions(ctx context.Context) ([]ProxyIPOption, error)
...
func (s *adminServiceImpl) ListAccountsForSchedulerScoreFilter(ctx context.Context, platform, accountType, status, search string, groupID int64, privacyMode, networkStatus, exitIP, rtStatus, capacityStatus string) ([]Account, error) {
	...
	if strings.TrimSpace(capacityStatus) == AccountCapacityStatusConcurrentFilter {
		if s.concurrencyService == nil {
			return nil, fmt.Errorf("concurrency service unavailable")
		}
		ids, err := s.concurrencyService.ListActiveAccountIDs(ctx)
		if err != nil {
			return nil, fmt.Errorf("list active concurrent accounts: %w", err)
		}
		if ids == nil {
			return nil, fmt.Errorf("concurrency service unavailable")
		}
	...
	return s.accountRepo.ListAllWithFilters(ctx, platform, accountType, status, search, groupID, privacyMode, networkStatus, exitIP, rtStatus, capacityStatus, accountIDs)
}
...
func (s *adminServiceImpl) GetProxyIPOptions(ctx context.Context) ([]ProxyIPOption, error) {
	return s.proxyRepo.ListIPOptions(ctx)
}
