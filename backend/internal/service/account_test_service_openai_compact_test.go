...
func TestAccountTestService_OpenAICompact401SuppressesPermanentErrorWhenContextRequestsProbeOnly(t *testing.T) {
	gin.SetMode(gin.TestMode)
	updateCalls := make(chan map[string]any, 1)
	account := Account{ID: 7, Name: "openai-compact-oauth", Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Credentials: map[string]any{"access_token": "oauth-token"}}
	repo := &compactStatusMutationRepo{snapshotUpdateAccountRepo: snapshotUpdateAccountRepo{stubOpenAIAccountRepo: stubOpenAIAccountRepo{accounts: []Account{account}}, updateExtraCalls: updateCalls}}
	upstream := &httpUpstreamRecorder{resp: &http.Response{StatusCode: http.StatusUnauthorized, Header: http.Header{"Content-Type": []string{"application/json"}}, Body: io.NopCloser(strings.NewReader(`{"error":"bad token"}`))}}
	svc := &AccountTestService{accountRepo: repo, httpUpstream: upstream}
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/v1/admin/accounts/7/test", bytes.NewReader(nil)).WithContext(accountTestSuppressStatusMutation(context.Background()))
	err := svc.TestAccountConnection(c, account.ID, "gpt-5.4", "", AccountTestModeCompact)
	require.Error(t, err)
...
func TestAccountTestService_OpenAICompact429SuppressesRateLimitWhenContextRequestsProbeOnly(t *testing.T) {
	gin.SetMode(gin.TestMode)
	updateCalls := make(chan map[string]any, 1)
	account := Account{ID: 8, Name: "openai-compact-oauth", Platform: PlatformOpenAI, Type: AccountTypeOAuth, Status: StatusActive, Schedulable: true, Concurrency: 1, Credentials: map[string]any{"access_token": "oauth-token"}}
	repo := &compactStatusMutationRepo{snapshotUpdateAccountRepo: snapshotUpdateAccountRepo{stubOpenAIAccountRepo: stubOpenAIAccountRepo{accounts: []Account{account}}, updateExtraCalls: updateCalls}}
	upstream := &httpUpstreamRecorder{resp: &http.Response{StatusCode: http.StatusTooManyRequests, Header: http.Header{"Content-Type": []string{"application/json"}}, Body: io.NopCloser(strings.NewReader(`{"error":{"type":"usage_limit_reached","resets_at":1777283883}}`))}}
	svc := &AccountTestService{accountRepo: repo, httpUpstream: upstream}
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/api/v1/admin/accounts/8/test", bytes.NewReader(nil)).WithContext(accountTestSuppressStatusMutation(context.Background()))
	err := svc.TestAccountConnection(c, account.ID, "gpt-5.4", "", AccountTestModeCompact)
	require.Error(t, err)
	require.Zero(t, repo.rateLimitedID)
	require.Zero(t, repo.setErrorID)
...
func (r *compactStatusMutationRepo) SetError(_ context.Context, id int64, errorMsg string) error {
	r.setErrorID, r.setErrorMsg = id, errorMsg
	return nil
}
...
func (r *compactStatusMutationRepo) ClearError(_ context.Context, _ int64) error { return nil }
