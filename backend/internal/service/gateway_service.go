...
func (s *GatewayService) initDebugGatewayBodyFile(path string) {
	...
	// #nosec G703 -- path comes from operator-only SUB2API_DEBUG_GATEWAY_BODY debug configuration.
	...
	if dir := filepath.Dir(path); dir != "." {
		// #nosec G703 -- dir is derived from operator-only debug log configuration.
	...
	// #nosec G703 -- path is an operator-only debug log destination.
