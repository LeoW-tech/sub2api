...
func TestAdminService_CreateCompositeGroupPreservesLive(t *testing.T) {
	repo := &groupRepoStubForAdmin{}
	svc := &adminServiceImpl{groupRepo: repo}
	group, err := svc.CreateGroup(context.Background(), &CreateGroupInput{Name: "composite-group", Platform: PlatformComposite, RateMultiplier: 1.0, AllowLive: true})
	require.NoError(t, err)
	require.NotNil(t, group)
	require.NotNil(t, repo.created)
...
func TestAdminService_UpdateCompositeGroupPreservesLive(t *testing.T) {
	existingGroup := &Group{ID: 1, Name: "composite-group", Platform: PlatformComposite, Status: StatusActive}
	repo := &groupRepoStubForAdmin{getByID: existingGroup}
	svc := &adminServiceImpl{groupRepo: repo}
	allowLive := true
	group, err := svc.UpdateGroup(context.Background(), existingGroup.ID, &UpdateGroupInput{AllowLive: &allowLive})
	require.NoError(t, err)
	require.NotNil(t, group)
	require.NotNil(t, repo.updated)
