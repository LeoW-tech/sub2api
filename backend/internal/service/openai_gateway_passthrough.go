...
func (s *OpenAIGatewayService) forwardOpenAIPassthrough(
	...
	// Passthrough can be invoked directly by tests/internal callers, bypassing the HTTP handler's
	// compact whitelist normalization. Normalize here as a defensive boundary as well.
	if isOpenAIResponsesCompactPath(c) {
	...
	// Keep compact requests on the strict upstream whitelist after every body rewrite.
	// This prevents later policy/compat steps from reintroducing request-scoped fields.
	if isOpenAIResponsesCompactPath(c) {
		normalizedBody, normalized, err := normalizeOpenAICompactRequestBody(body)
		if err != nil {
			return nil, err
