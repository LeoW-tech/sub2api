...
func TestApiKeyAuthWithSubscriptionGoogle_InsufficientBalance(t *testing.T) {
	...
	require.Equal(t, "余额不足，请充值或联系管理员（Insufficient account balance. Please top up or contact administrator.）", resp.Error.Message)
...
func TestApiKeyAuthWithSubscriptionGoogle_RejectsExhaustedBalance(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal(rec.Body.Bytes(), &resp))
	require.Equal(t, http.StatusForbidden, resp.Error.Code)
	require.Equal(t, insufficientBalanceMessage, resp.Error.Message)
	require.Equal(t, "PERMISSION_DENIED", resp.Error.Status)
}
